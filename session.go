...
import (
	...
	SCEncoder "github.com/lucas-clemente/quic-go/SCEncoder"
...
func (s *session) sendPackedPacket(packet *packedPacket, now time.Time) {
	...
	if (packet.EncryptionLevel() == protocol.Encryption1RTT || packet.EncryptionLevel() == protocol.Encryption0RTT) && s.SCEncoderenable {
		...
		cpkt := s.SCEncoder.Enqueue_Packet(pktdata, int(QUICID))
		SCstr := []byte("SC")
		SCstr = append(SCstr, cpkt...)
		packet.buffer.Data = SCstr
		s.sendQueue.Send(packet.buffer)
		repairedata, ok := s.SCEncoder.RetrieveCodedPackets()
		if ok {
			for {
				if s.sendQueue.WouldBlock() {
					continue
				...
				SCstr := []byte("SC")
				SCstr = append(SCstr, repairedata...)
				packetbuffer := getPacketBuffer()
				packetbuffer.Data = SCstr
				s.sendQueue.Send(packetbuffer)
				break
			}
		}
	} else {
