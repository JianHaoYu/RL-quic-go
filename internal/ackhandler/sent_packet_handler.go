...
func (h *sentPacketHandler) detectLostPackets(now time.Time, encLevel protocol.EncryptionLevel) error {
	...
	return pnSpace.history.Iterate(func(p *Packet) (bool, error) {
		...
		if p.SendTime.Before(lostSendTime) {
			packetLost = true
			fmt.Printf("[LOST] lostpacket %d (timethreshold) ,AtTime: %f \n", p.PacketNumber, float64(time.Now().UnixNano()/1e3)/1e6)
			if h.logger.Debug() {
			...
			packetLost = true
			fmt.Printf("[LOST] lostpacket %d (reorderingthreshold) ,AtTime: %f ,largestAcked: %d \n", p.PacketNumber, float64(time.Now().UnixNano()/1e3)/1e6, pnSpace.largestAcked)
			if h.logger.Debug() {
